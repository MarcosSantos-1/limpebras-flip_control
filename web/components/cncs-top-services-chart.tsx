...
export interface CNCsTopServiceDatum {
  tipoServico: string;
  quantidade: number;
}

interface CNCsTopServicesChartProps {
  data: CNCsTopServiceDatum[];
  monthLabel: string;
  onPrevMonth: () => void;
  onNextMonth: () => void;
...
export function CNCsTopServicesChart({
  data,
  monthLabel,
  onPrevMonth,
  onNextMonth,
  disableNextMonth,
}: CNCsTopServicesChartProps) {
  ...
  if (!mounted) {
    return (
      <Card className="h-full">
        <CardHeader>
          <CardTitle className="text-lg font-semibold text-emerald-600">Top Serviços BFS</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const chartData = {
    labels: topData.map((d) =>
      d.tipoServico.length > 40 ? `${d.tipoServico.slice(0, 40)}...` : d.tipoServico
    ),
    datasets: [
      {
        label: "Quantidade",
      ...
      <CardHeader className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>Tipos de serviço BFS com maior volume no período</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        {!hasData ? (
          <div className="h-64 flex items-center justify-center text-muted-foreground">
            Sem dados de BFS para este período.
          </div>
        ) : (
          <div className="h-72">
